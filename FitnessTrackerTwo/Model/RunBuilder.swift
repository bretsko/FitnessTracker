...
class RunBuilder {
	...
	private func compactLastDetails(flush: Bool = false) {
		...
		if let index = rawDetails.firstIndex(where: { $0.start >= lastCompactedEnd }) {
	...
	private func addRawDetail(distance: Double, start: Date, end: Date) {
		...
		if let index = rawDetails.firstIndex(where: { end.timeIntervalSince($0.start) < paceTimePrecision }) {
			let range = rawDetails.suffix(from: index)
